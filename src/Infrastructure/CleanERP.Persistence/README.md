# CleanERP.Persistence Layer

This is the Persistence layer of the CleanERP system, responsible for data access and database operations using Entity Framework Core with PostgreSQL.
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Database=CleanERP;Username=postgres;Password=your_password"
  }
}
```
...
### Add Migration
```bash
dotnet ef migrations add InitialCreate --project src/Infrastructure/CleanERP.Persistence --startup-project src/Presentation/CleanERP.API
```

### Update Database
```bash
dotnet ef database update --project src/Infrastructure/CleanERP.Persistence --startup-project src/Presentation/CleanERP.API
```
